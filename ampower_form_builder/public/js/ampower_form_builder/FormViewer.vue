<template>
	<div class="fv-layout-container" :class="{ 'is-embedded': embeddedMode }">
		<div class="fv-main-form">
			<div class="fv-container fv-view-shell" :class="{ 'is-embedded': embeddedMode }">
				<div v-if="!embeddedMode" class="fv-view-header">
					<div class="fv-view-heading">
						<div class="fv-view-kicker">{{ viewKicker }}</div>
						<h2 class="fv-view-title">{{ pageTitle }}</h2>
						<p v-if="headerDescription" class="fv-view-description">{{ headerDescription }}</p>
					</div>
					<div class="fv-view-actions">
						<template v-if="reportPreviewOpen">
							<button type="button" class="fv-action-btn secondary" :title="__('Back to Form')" @click="closeReport">
								{{ __("Back to Form") }}
							</button>
							<button type="button" class="fv-action-btn secondary" :title="__('Print')" @click="printReport('fv-print-report-preview')">
								{{ __("Print") }}
							</button>
							<button type="button" class="fv-action-btn primary" :title="__('Refresh Report')" @click="createReport">
								{{ __("Refresh Report") }}
							</button>
						</template>
						<template v-else-if="isFormsDirectoryView">
							<button type="button" class="fv-action-btn secondary" :title="__('Open Builder')" @click="openBuilder">
								{{ __("Open Builder") }}
							</button>
						</template>
						<template v-else-if="isReportsListView">
							<button type="button" class="fv-action-btn secondary" :title="__('Back to Forms')" @click="openDirectory">
								{{ __("Back to Forms") }}
							</button>
							<button type="button" class="fv-action-btn primary" :title="__('Add Entry')" @click="resetEntry">
								{{ __("Add Entry") }}
							</button>
						</template>
						<template v-else>
							<button type="button" class="fv-action-btn secondary" :title="__('Back to Reports')" @click="openSubmissionList">
								{{ __("Back to Reports") }}
							</button>
							<button
								v-if="loadedSubmissionName"
								type="button"
							...
							/>
							<div class="fv-print-controls">
								<select
									v-model="selectedPrintFormatName"
									class="form-control fv-print-format-select"
									:disabled="printFormatsLoading || !printFormats.length"
								>
									<option value="">{{ __("Select Print Format") }}</option>
									<option v-for="item in printFormats" :key="item.name" :value="item.name">
										{{ item.format_name }}
									</option>
								</select>
								<button
									type="button"
									class="fv-action-btn primary"
									:disabled="!canCreateReport || reportLoading"
									:title="__('Create Report')"
									@click="createReport"
								>
									{{ reportLoading ? __("Creating...") : __("Create Report") }}
								</button>
							</div>
							<button type="button" class="fv-action-btn primary" :title="primaryActionLabel" @click="handlePrimaryAction">
								{{ primaryActionLabel }}
							</button>
						</template>
					</div>
				...
				<div class="fv-view-body">
					...
					/>
					<div v-else-if="isReportsListView" class="fv-records-view">
						<div class="fv-records-toolbar">
							<div class="fv-records-search">
								<input
									v-model="submissionSearch"
									class="form-control fv-submission-search"
									type="text"
									:placeholder="__('Search reports')"
								>
							</div>
							<div class="fv-records-count">{{ __('{0} report(s)', [filteredSubmissions.length]) }}</div>
						</div>
						<div class="fv-records-panel">
							<div class="fv-records-head">
								<span>{{ __("Title") }}</span>
								<span>{{ __("Status") }}</span>
								<span>{{ __("Created On") }}</span>
							</div>
							<div v-if="loadingSubmissions" class="fv-state-card fv-state-loading">
								<div class="fv-state-title">{{ __("Loading reports...") }}</div>
								<div class="fv-state-copy">{{ __("Fetching saved entries for this form.") }}</div>
							</div>
							<template v-else-if="filteredSubmissions.length">
								<button
									v-for="submission in filteredSubmissions"
									:key="submission._name"
									type="button"
									class="fv-record-row"
									@click="viewSubmission(submission._name)"
								>
									<span class="fv-record-title">
										<strong>{{ getSubmissionTitle(submission) }}</strong>
										<small>{{ submission._name }}</small>
									</span>
									<span class="fv-record-status" :class="`is-${getSubmissionStatusTone(submission)}`">
										{{ getSubmissionStatusLabel(submission) }}
									</span>
									<span class="fv-record-date">{{ submission._submitted_on || __('Not available') }}</span>
								</button>
							</template>
							<div v-else class="fv-state-card fv-state-empty">
								<div class="fv-state-title">{{ __("No reports found") }}</div>
								<div class="fv-state-copy">{{ __("Create the first entry for this form to start collecting reports.") }}</div>
							</div>
						</div>
					</div>
					<Form
...
const NEW_SUBMISSION_ROUTE_TOKEN = "new";
const viewMenuItems = computed(() => {
	const items = [{ key: "build_new_form", label: __("Build New Form") }];
...
const headerDescription = computed(() => {
	if (isFormsDirectoryView.value) return __("Choose a form to open its report entries.");
	if (isReportsListView.value) return templateDescription.value || __("Select a saved report entry or create a new one.");
	if (creatingNewEntry.value) return __("Create a new report entry for this form.");
	if (activeSubmission.value) return __("Viewing report entry {0}.", [activeSubmission.value._name]);
	return templateDescription.value || __("Review or update the selected report entry.");
});
const primaryActionLabel = computed(() => loadedSubmissionName.value ? __("Update Form") : __("Submit Form"));
...
function getSubmissionMethod() {
	return loadedSubmissionName.value
...
function fetchActiveTemplates() {
	frappe.call({
		...
		error: (error) => reportViewerError(__("Load Forms Error"), error, __("Unable to load forms.")),
	});
}

function loadTemplate(templateName, { submissionName = "" } = {}) {
...
function fetchSubmissions() {
	...
	frappe.call({
		...
		args: { form_template: templateDocName.value },
		callback: (response) => {
...
function loadSubmissionByName(name, { announce = true } = {}) {
	return frappe.call({
		...
		return withHydrationGuard(async () => {
			if (
				templateDocName.value
...
function viewSubmission(name) {
	...
	if (templateDocName.value) {
		frappe.set_route("form_viewer", templateDocName.value, name);
	}
}
...
function resetEntry({ updateRoute = true } = {}) {
	...
	if (updateRoute && !embeddedMode.value && templateDocName.value) {
		frappe.set_route("form_viewer", templateDocName.value, NEW_SUBMISSION_ROUTE_TOKEN);
	}
}
...
function openTemplate(name) {
	...
	if (props.templateName !== name) {
		frappe.set_route("form_viewer", name);
		return;
	}
...
function openDirectory() {
	frappe.set_route("form_viewer");
}

function openSubmissionList() {
	if (!embeddedMode.value && templateDocName.value) {
		frappe.set_route("form_viewer", templateDocName.value);
	}
}
...
async function createReport({ silent = false } = {}) {
...
async function handleSubmissionSaved(submissionName) {
	...
	if (!embeddedMode.value && templateDocName.value) {
		frappe.set_route("form_viewer", templateDocName.value, submissionName);
	}
...
function submitForm() {
	...
	frappe.call({
		...
		error: (error) => reportViewerError(__("Submit Error"), error, __("Unable to submit form.")),
	});
}

function updateForm() {
	const payload = getPayload();
	if (!payload || !loadedSubmissionName.value) return;
	...
	frappe.call({
		...
		callback: async (response) => {
			...
			try {
				await handleSubmissionSaved(response.message.name || loadedSubmissionName.value);
			} catch (error) {
				reportViewerError(__("Submission Link Error"), error, __("Unable to link the submission to the parent document."));
			}
			if (embeddedMode.value) return;
			fetchSubmissions();
			viewSubmission(loadedSubmissionName.value);
		},
		error: (error) => reportViewerError(__("Update Error"), error, __("Unable to update form.")),
	});
}
...
async function syncSubmission(options = {}) {
	...
	if (embeddedMode.value && !loadedSubmissionName.value) {
	...
	if (hasExistingSubmission) {
		args.submission_name = loadedSubmissionName.value;
	}
	if (options.status) {
	...
	try {
		...
		const submissionName = response.message?.name || loadedSubmissionName.value || "";
		if (submissionName) {
			loadedSubmissionName.value = submissionName;
		}
		return submissionName;
...
.fv-record-row {
	display: grid;
	grid-template-columns: minmax(0, 1.8fr) minmax(110px, 0.6fr) minmax(150px, 0.8fr);
	gap: 1rem;
	align-items: center;
}
...
.fv-record-title strong,
.fv-record-title small,
.fv-record-date {
	overflow: hidden;
...
.fv-record-status.is-submitted {
	background: rgba(22, 163, 74, 0.12);
	color: #15803d;
}

.fv-record-status.is-draft,
.fv-record-status.is-neutral {
	background: rgba(59, 130, 246, 0.12);
	color: #1d4ed8;
}

.fv-record-status.is-cancelled {
