		...
		<div v-if="open" class="afb-ai-modal">
			...
			<div class="afb-ai-shell" role="dialog" aria-modal="true">
				<header class="afb-ai-header">
					<div>
						<div class="afb-ai-kicker">{{ t("AI Agent") }}</div>
						<h2>{{ t("Add via AI Agent") }}</h2>
						<p>
							{{ t("Upload an image or a PDF with up to 5 pages. We will OCR the pages with Google Vision and draft the form structure for you.") }}
						</p>
					</div>
				...
				<div class="afb-ai-body">
					<section class="afb-ai-steps">
						<div class="afb-ai-step">
							...
							<div>
								<h4>{{ t("Upload") }}</h4>
								<p>{{ t("Choose a PDF or image. The file stays private in your site and we preserve table regions from report-style documents.") }}</p>
							</div>
						</div>
						<div class="afb-ai-step">
							...
							<div>
								<h4>{{ t("OCR") }}</h4>
								<p>{{ t("Google Vision extracts the text and layout from every page, including repeating grid blocks.") }}</p>
							</div>
						</div>
						<div class="afb-ai-step">
							...
							<div>
								<h4>{{ t("Draft") }}</h4>
								<p>{{ t("OpenAI gpt-5-mini converts the OCR into a builder-ready form schema with tables kept as tables.") }}</p>
							</div>
						</div>
					</section>
					...
					<section class="afb-ai-panel">
						...
						<div v-else class="afb-ai-progress-card">
							...
							<div v-if="stage === 'error'" class="afb-ai-error">
								<div class="afb-ai-error-title">{{ t("We could not finish the import") }}</div>
								<p>{{ errorMessage }}</p>
...
function startUpload(file) {
	...
	xhr.onreadystatechange = async () => {
		...
		statusMessage.value = t("File uploaded. Queueing background analysis...");
		await queueImport(fileDoc.name, token);
	};
...
async function queueImport(fileDocname, token) {
	if (!isCurrentWorkflow(token)) return;
	try {
		const { message } = await frappe.call({
			method: "ampower_form_builder.api.enqueue_ai_form_builder_import",
			args: { file_docname: fileDocname },
			quiet: true,
		});
		if (!isCurrentWorkflow(token)) return;
		if (!message?.job_id) {
			throw new Error(t("The server did not return a job id."));
		}
		jobId.value = message.job_id;
		processingProgress.value = 5;
		statusMessage.value = t("Queued for processing...");
		...
		if (!isCurrentWorkflow(token)) return;
		stage.value = "error";
		errorMessage.value = error?.message || t("Unable to start the AI import.");
		statusMessage.value = errorMessage.value;
	}
}
...
function startPolling(token) {
	stopPolling();
	pollTimer.value = window.setInterval(() => refreshStatus(token), 2000);
}
...
async function refreshStatus(token) {
	if (!isCurrentWorkflow(token) || !jobId.value || stage.value === "error") return;

	try {
		const { message } = await frappe.call({
			method: "ampower_form_builder.api.get_ai_form_builder_import_status",
			args: { job_id: jobId.value },
			quiet: true,
		});

		if (!isCurrentWorkflow(token) || !message) return;

		processingProgress.value = Number(message.progress || processingProgress.value || 0);
		statusMessage.value = message.message || statusMessage.value;
		...
		if (message.status === "failed") {
			...
			stopPolling();
			stage.value = "error";
			errorMessage.value = message.error || message.message || t("The AI import failed.");
			statusMessage.value = errorMessage.value;
		}
	} catch (error) {
		// Keep polling if the connection is flaky. The background job still runs.
	}
}
...
function isCurrentWorkflow(token) {
	return token === workflowToken.value;
}

function parseUploadError(responseText) {
	try {
		const response = JSON.parse(responseText);
		if (response?._error_message) return response._error_message;
		if (response?._server_messages) {
			const messages = JSON.parse(response._server_messages).map((item) => JSON.parse(item).message).filter(Boolean);
